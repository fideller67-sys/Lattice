...
import { Layout, Server, Activity, Database, GitMerge, MoreHorizontal, Loader2 } from 'lucide-react';
import api from '../../config/api';

export default function DirectorPlatformEng() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get('/metric-dashboards/platform-eng');
        setData(res);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <span className="text-gray-500 font-light">#</span> Platform Engineering
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-[#111116] border border-white/5 rounded-xl p-6">
          ...
          <div className="text-xs text-blue-400 mt-2 font-medium">{metrics[1].subtext}</div>
        </div>
        <div className="bg-[#111116] border border-white/5 rounded-xl p-6">
          <div className="flex items-center gap-3 mb-2">
          ...
          <div className="text-xs text-emerald-400 mt-2 font-medium">{metrics[2].subtext}</div>
        </div>
      </div>
      ...
      <div className="bg-[#111116] border border-white/5 rounded-xl overflow-hidden">
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <h2 className="text-sm font-bold text-white">Service Health & Pipelines</h2>
          <button className="text-xs font-medium text-blue-400 hover:text-blue-300 transition-colors">View Architecture</button>
        </div>
        <div className="divide-y divide-white/5">
          {pipelines.map((pipe) => (
            <div key={pipe.id} className="p-5 hover:bg-white/[0.02] transition-colors flex items-center justify-between group">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-mono text-gray-500">{pipe.id}</span>
                    <h3 className="text-sm font-bold text-white group-hover:text-blue-400 transition-colors">{pipe.name}</h3>
                  </div>
                  <div className="flex items-center gap-4 text-xs font-medium text-gray-500">
                    <span>Stage: <span className="text-gray-300">{pipe.stage}</span></span>
                    <span>•</span>
                    <span>Uptime: {pipe.uptime} ({pipe.deployments} deploys)</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6">
                <div className="text-right">
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${pipe.statusColor}`}>
                    {pipe.status}
                  </span>
